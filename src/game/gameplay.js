...
export class Game {
  ...
  /* Event funnel: gameplay emits named events; the engine matches them against
     the active mission's event/count objectives (payload matched against
     `special`). A no-op in free survey — no mission, nothing to complete. */
  emit(event, payload = null) {
    const m = this.mission;
    if (!m) return;
    for (const o of m.objectives) {
      if ((o.type !== 'event' && o.type !== 'count') || o.on !== event) continue;
      if (o.special && o.special !== payload) continue;
      if (o.type === 'count') this.bump(o.id);
      else this.complete(o.id);
    }
  }

  /* State objectives, checked every frame against the world. All parameters
     (landmark, comparison, distance, min ground height) come from the
     objective data — no ids or literals here. The two log/unlock beats are
     narrative, keyed to the landmark they fire at. */
  checkState() {
    const m = this.mission;
    if (!m) return;
    ...
    for (const o of m.objectives) {
      ...
      if (o.ref === 'station') {
        this.unlock('roster');
        this.log('VANTAGE-3 PERIMETER — NO SEISMIC SIGNATURE', 'warn');
      } else if (o.ref === 'massif') {
        this.log('CENTRAL MASSIF — NETWORK CONVERGENCE POINT');
      }
    }
  }

  /* Content gating: a tag is OPEN while the active mission has an unmet
     objective declaring it. That is what keeps the node drillable — its own
     objective must still be open — and what closes the station prompt once
     the logs are recovered. */
  tagOpen(tag) {
    const m = this.mission;
    if (!m) return false;
    return m.objectives.some(o => o.unlocks === tag && !this.objDone[o.id]);
  }

  anomalyOpen(a) {
    return !a.unlocks || this.tagOpen(a.unlocks);
  }
  ...
  _finishScan() {
    ...
    for (const a of this.anoms) {
      ...
      if (!this.anomalyOpen(a)) continue;
  ...
  addMarker(a) {
    ...
    const ring = new THREE.Mesh(new THREE.RingGeometry(0.85, 1.05, 26),
      new THREE.MeshBasicMaterial({ color: a.special || a.type === 'pipe' ? 0xffb454 : 0x2ad2ff,
  ...
  togglePanel() {
    ...
    this.audio.ui('tick');
    if (this.rover.panelTarget > 0.5) this.emit('array-deployed');
  }
  ...
  save() {
    const blob = {
      ...
      // [anomalyId, 1=taken | 2=found] for non-default states only
