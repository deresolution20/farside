/* ============================================================
   THINGS ON THE FLOOR OF ANAXIMENES
   ------------------------------------------------------------
   Boulder fields (instanced), the descent sled you arrived on,
   what is left of VANTAGE-3, survey pylons, deployable relays,
...
export class Props {
  ...
  /* ============================================================
     ...
     ============================================================ */
  buildPipeNode(x, z, scale = 1, big = false) {
    const g = new THREE.Group();
    ...
    (this.pipes ||= []).push(g);
    this.colliders.push({ x, z, r: 0.9 * scale, kind: 'pipe' });
    return g;
  }
  ...
  update(dt, t, camera) {
    ...
    if (this.pipes) for (const l of this.pipes) {
...
function makeFlagMaterial() {
  ...
  g.fillText('MERIDIAN', 26, 62);
  g.font = '500 14px ui-monospace, monospace';
  g.fillStyle = '#6fe3f5';
  g.fillText('AUTHORITY', 26, 86);
  g.fillStyle = '#8b8578';
