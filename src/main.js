/* ============================================================
   FARSIDE — The Knocking at Anaximenes
...
async function boot() {
  ...
  window.FARSIDE = App;                 // debug handle: inspect or drive from the console
...
function startGame(freeRoam, loadSaved) {
  ...
  App.hud.log('K-9 KESTREL — SYSTEMS NOMINAL', 'good');
  App.hud.log('MERIDIAN AUTHORITY · NW-LIMB SURVEY DIVISION');

  if (!resumed && !freeRoam) {
    setTimeout(() => {
      App.hud.showCard(App.game.mission); App.state = ST.CARD;
...
function tick(dt) {
  ...
  try {
    ...
    // A mid-frame throw (a transient GL hiccup on software rendering) must not
    // strand the edge set: the next frame would re-read the same keypress and
    // double-fire it — one tap of R flipped the arm out and back in.
    input.endFrame();
  }
