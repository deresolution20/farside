...
`FARSIDE.tick(dt)` applies **no dt clamp** — only the real `frame()` does. Meanwhile
...
The *logic* is data-driven (Phase 1). Every objective in `MISSIONS` is a typed record —
`{ type: event | distance | count, …params }` — completed by a small engine in `gameplay.js`:
`emit(event, payload)` is the single funnel for event/count objectives (action sites call it,
they no longer call `complete('<id>')` directly), and `checkState()` evaluates `distance`
records against `LANDMARKS` (`content.js`) each frame. There are no positional `missionIdx`
gates — the drumhead lock, the station prompt, and the compass/minimap POIs all derive from the
current mission's *unmet* objectives (`tagOpen` / `anomalyOpen` / `objectiveTargets`).
...
`save()` writes each anomaly as an `[id, 0|1|2]` pair (non-default states only), where
`a.id = round(x*10)+','+round(z*10)` — coordinate-derived and stable **only while
`buildAnomalies()` is deterministic** (fixed seed, fixed terrain). `load()` re-keys by id and
silently ignores unknown ids.

So: change the terrain, a filter, the push order, or the count, and the ids move. Old saves
then either drop state (an id no longer matches) or — worse — land it on a *different* object
that happens to share a rounded coordinate. Verified in Phase 0, a one-slot positional shift
